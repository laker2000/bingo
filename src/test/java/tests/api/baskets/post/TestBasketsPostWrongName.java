...
public class TestBasketsPostWrongName extends TestConfig {
    ...
    /**
     * Scenario: Check creation of a basket with with wrong name values in order to provoke Bad Request 400 response
     *
     * Test steps:
     *
     * 1. Arrange a basket for creation
     * 2. Try to create a basket with wrong name parameter
    ...
    /**
     * Scenario: Check creation of a basket with with wrong name values in order to provoke NOT FOUND 404 response
     *
     * Test steps:
     *
     * 1. Arrange a basket for creation
    ...
    public void testBasketsPostWrongNameNotFound(){
        ...
        List<String> wrongNames = Arrays.asList("", "<script>alert('Injected!');</script>");
        RequestSpecBuilder builder = new RequestSpecBuilder();
        builder.addHeader("Authorization", serviceToken);
        var requestSpec = builder.build();
        var request = RestAssured.given().spec(requestSpec);
